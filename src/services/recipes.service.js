...
export function requestRecipeById(token, recipe) {
    return fetch(`${config.recipesAPI}/get-recipe?=${recipe.id}`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer ' + token
...
export function deleteRecipe(token, recipe) {
    return fetch(`${config.recipesAPI}/delete-recipe?id=${recipe.id}`, {
        method: 'DELETE',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer ' + token

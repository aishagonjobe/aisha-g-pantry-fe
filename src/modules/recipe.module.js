...
const initialState = {
    ...
    updateRecipeCategoryPending: false,
    updateRecipeCategoryFailure: false,
    updateRecipeCategorySuccess: false,
    ...
    return function getRecipe(dispatch, getState) {
        ...
        requestRecipe(getState().user.token).then(response => {
            ...
            response.json().then(json => {
                ...
                dispatch(getRecipeSuccess(json.recipe_list))
            })
        })
    }
}

export function initiateCreateRecipe(recipe) {
    return function createRecipeDispatcher(dispatch, getState) {
        dispatch(createRecipeRequest())
        createRecipe(getState().user.token, recipe).then(response => {
            if (!response.ok) {
...
export function initiateUpdateRecipe(recipe) {
    return function updateRecipeDispatcher(dispatch, getState) {
        dispatch(updateRecipeRequest())
        updateRecipe(localStorage.getItem("token"), recipe).then(response => {
            if (!response.ok) {
...
export function initiateDeleteRecipe(recipe) {
    return function deleteRecipeDispatcher(dispatch, getState) {
        dispatch(deleteRecipeRequest())
        deleteRecipe(getState().user.token, recipe).then(response => {
            if (!response.ok) {
...
export function initiateUpdateRecipeCategory(recipeCategory) {
    return function updateRecipeCategoryDispatcher(dispatch, getState) {
        ...
        updateRecipeCategory(localStorage.getItem("token"), recipeCategory).then(response => {
            if (!response.ok) {

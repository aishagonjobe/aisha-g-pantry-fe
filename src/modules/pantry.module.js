...
function getFoodSuccess(food, favorite) {
    return {
    ...
    return function getFood(dispatch, getState) {
        ...
        requestFood(getState().user.token).then(response => {
            ...
            response.json().then(json => {
                ...
                dispatch(getFoodSuccess(json))
            })
        })
    }
}

export function initiateCreateFood(foodItem) {
    return function createFoodDispatcher(dispatch, getState) {
        dispatch(createFoodRequest())
        createFood(getState().user.token, foodItem).then(response => {
            if (!response.ok) {
...
export function initiateUpdateFood(foodItem) {
    return function updateFoodDispatcher(dispatch, getState) {
        dispatch(updateFoodRequest())
        updateFood(getState().user.token, foodItem).then(response => {
            if (!response.ok) {
...
export function initiateDeleteFood(foodItem) {
    return function deleteFoodDispatcher(dispatch, getState) {
        dispatch(deleteFoodRequest())
        deleteFood(getState().user.token, foodItem).then(response => {
            if (!response.ok) {
